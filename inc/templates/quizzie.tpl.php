		...
		<?php for ( $i = 0; $i < count( $options ); $i++ ): ?>
			<div class="quizzie-result" data-option="<?php echo esc_attr( $i ) ?>">
				...
				<div class="quizzie-result-share">
					<a href="<?php echo esc_attr( 'https://www.facebook.com/sharer/sharer.php?u=' . urlencode( get_permalink() ) ) ?>" target="_blank" class="quizzie-share-facebook button">Share on Facebook</a>
					<a href="<?php echo esc_attr( 'https://twitter.com/share?text=' . urlencode( $question ) ) ?>" target="_blank" class="quizzie-share-twitter button">Share on Twitter</a>
				</div>
			</div>
		<?php endfor; ?>
	</div>
